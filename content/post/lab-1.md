...
````
cf login -a https://api.run.haas-99.pez.pivotal.io --skip-ssl-validation
  Email: student1
  Password: ••••••••
...
Login to the App Console at https://apps.run.haas-99.pez.pivotal.io
...
1. Push the cities-hello, put your initials in the app name so we don't get conflicts
    ...
    urls: cities-hello-lactiferous-unanswerableness.run.haas-99.pez.pivotal.io
...
3. If you haven't already it is a good time to walk through the AppsManager:
        ...
        https://apps.run.haas-99.pez.pivotal.io
...
2. Create a mysql service, name it as `<YOUR INITIALS>-cities-db`
    ...
    You can create the service from the `cli` or launch the App Manager-> Select the Development Space [https://apps.run.haas-99.pez.pivotal.io](https://apps.run.haas-99.pez.pivotal.io) and login.
...
6. Verify you can access your application via a curl request or your browser:
   You will have to get the route to your app
    ...
    ````bash
       ...
       $ cf apps
          url: instructor-cities-service.run.haas-99.pez.pivotal.io  
          // Note - Use HTTPS
       $ curl -i -k https://instructor-cities-service.run.haas-99.pez.pivotal.io
    ````

    For Windows
    ````
       Open the URL (e.g. https://instructor-cities-service.run.haas-99.pez.pivotal.io) in a browser window
    ````
    We must be able to access your application at https://instructor-cities-service.run.haas-99.pez.pivotal.io for the next steps to work properly.
...
Open the metrics dashboard at https://metrics.run.haas-99.pez.pivotal.io/
...
You will get the output similar to this on your terminal
...

  {
   "VCAP_APPLICATION": {
    ...
    "application_uris": [
     "rj-cities-service.haas-99.pez.pivotal.io"
    ],
...
To verify that the application is running, use the following curl commands (or use your browser) to retrieve data from the service or use a browser to access the URL:
  ...
  ````bash
  $ curl -i -k https://<studentXX>-cities-service.cfapps.haas-99.pez.pivotal.io/cities
  ````

  ````bash
  $ curl -i -k https://<studentXX>-cities-service.cfapps.haas-99.pez.pivotal.io/cities/49
  ````

  ````bash
  $ curl -i -k https://<studentXX>-cities-service.cfapps.haas-99.pez.pivotal.io/cities?size=5
  ````
...
3. You will need to specify the parameter citiesuri to the user defined service instance .
  ...
  citiesuri>   http://<studentXX>-cities-service.cfapps.haas-99.pez.pivotal.io/
  ...
  "user-provided": [
   {
    "credentials": {
     "tag": "cities",
     "uri": "https://rj-cities-service.haas-99.pez.pivotal.io/"
    },
    "label": "user-provided",
    "name": "cities-ws",
    "syslog_drain_url": "",
...

{
 "VCAP_APPLICATION": {
  ...
  "application_uris": [
   "rj-cities-ui.run.haas-99.pez.pivotal.io"
  ],
...
First push a new version of the app with a blue route.
    ...
    $ cf map-route studentXXX-cities-hello-v1 cfapps.haas-99.pez.pivotal.io --hostname studentXXX-cities-hello
...
Next, you can push a new version of the app with a green route.
    ...
    $ cf map-route studentXXX-cities-hello-v2 cfapps.haas-99.pez.pivotal.io --hostname studentXXX-cities-hello
    // Unmap the outside route to the blue version. All the traffic is now directed to v2
    $ cf unmap-route studentXXX-cities-hello-v1 cfapps.haas-99.pez.pivotal.io --hostname studentXXX-cities-hello
...
If you would like to inject build numbers in your app names here is a script you could use to do blue green deployments in the cities-hello directory which only works on a Mac
  ...
  $ ./blue-green.sh  cities-hello 1001 run.haas-99.pez.pivotal.io
  ...
  ````bash
  $ ./blue-green.sh  cities-hello 1002 run.haas-99.pez.pivotal.io
